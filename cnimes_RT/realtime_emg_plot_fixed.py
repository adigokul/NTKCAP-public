...
class EMGQtViewer(QtWidgets.QMainWindow):
    ...
    def _build_ui(self, show_marker=True, marker_sec=5.0):
        ...
        for i, name in enumerate(CHANNEL_NAMES):
            ...
            p.setLabel('left', name, **{'color': '#666', 'size': '9pt'})
            ...
            p.getAxis('left').setPen(pg.mkPen("#ffffff"))
            p.getAxis('bottom').setPen(pg.mkPen("#fc1414"))
            ...
            p.setYRange(-50, 50)  # default microvolt range; will autoscale
    ...
    def _refresh(self):
        ...
        for i, c in enumerate(self.curves):
            ...
            ymin = float(np.nanmin(y)) if y.size else -1.0
            ymax = float(np.nanmax(y)) if y.size else 1.0
            if np.isfinite(ymin) and np.isfinite(ymax) and ymax - ymin > 1e-9:
                span = ymax - ymin
                m = max(0.05 * span, 1e-3)
                self.plots[i].setYRange(ymin - m, ymax + m, padding=0.0)
