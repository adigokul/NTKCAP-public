...
class CameraProcess(Process):
    ...
    def run(self):
        ...
        while True:
            ...
            cap_e = time.time()
            #print(cap_e-cap_s)
            if cap_s != old_time: x = "FPS: " + str(1/(cap_s - old_time)) 
            else: x = "0"
            ...
            self.draw_frame(frame, keypoints, scores, palette, skeleton, link_color, point_color, x)
    ...
    def process_stop_record(self):
        pass
    def draw_frame(self, frame, keypoints, scores, palette, skeleton, link_color, point_color, fr):
        keypoints = keypoints.astype(int)
        cv2.putText(frame, fr, (10, 250), cv2.FONT_HERSHEY_SIMPLEX, 2, (30, 144, 255), 4, cv2.LINE_AA)
...
class UpdateThread(QThread):
    ...
    def __init__(self, shared_name, cam_id, start_evt, stop_evt, queue):
        super().__init__()
        ### define
        self.shared_name = shared_name
        self.start_evt = start_evt
...
class VideoPlayer(QThread):
    ...
    def run(self):        
        ...
        self.label1.setPixmap(self.np2qimage(self.result_load_videos_allframes[0][self.current_frame]))
        self.label2.setPixmap(self.np2qimage(self.result_load_videos_allframes[1][self.current_frame]))
        self.label3.setPixmap(self.np2qimage(self.result_load_videos_allframes[2][self.current_frame]))
        self.label4.setPixmap(self.np2qimage(self.result_load_videos_allframes[3][self.current_frame]))
    ...
    def slider_changed(self):        
        ...
        self.label3.setPixmap(self.np2qimage(self.result_load_videos_allframes[2][self.current_frame]))
        self.label4.setPixmap(self.np2qimage(self.result_load_videos_allframes[3][self.current_frame]))
        self.web_view_widget.page().runJavaScript(f"window.updateAnimationProgress({self.current_frame / (self.frame_count - 1)});")
    def result_web_load_finished(self):
        pass
...
class MainWindow(QMainWindow):
    ...
    def check_cal_finish(self):
        if not self.marker_calculate_process:
    ...
    def update_slider(self):
        self.result_video_slider.setValue(self.video_player.current_frame)
        self.frame_label.setText(str(self.video_player.current_frame))
    def select_result_cal_task(self, patient_path, result_select_patient_id, result_select_date, result_select_cal_time, result_select_task):
        ...
        self.result_video_slider.setMaximum(self.video_player.frame_count)
    ...
    def slider_changed(self, value):
        if self.is_playing:
            self.video_player.timer.stop()
            self.is_playing = False
        self.video_player.current_frame = value
    ...
    def closeEvent(self, event):
        self.closeCamera()
        if self.marker_calculate_process.is_alive():
