...
from easymocap.annotator.chessboard import getChessboard3d, findChessboardCorners,findChessboardCorners_manual
import numpy as np
...
def detect_chessboard(path, out, pattern, gridSize, args):
    ...
    for i in trange:
        ...
        if args.manual ==1:
            show = findChessboardCorners_manual(img, annots, pattern,imgname)
        else:
        ...
        while len(proposals) > 0:
            ...
            for nf in [left, right]:
                if not visited[nf]:
                    ...
                    annots = read_json(annotname)
                    if args.manual ==1:
                        show = findChessboardCorners_manual(img, annots, pattern)
                    else:
