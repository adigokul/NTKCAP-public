...
def find_real_dist_error(P,cam_coord,prep_4,Q4,prep_3,Q3,prep_2,Q2,Q3_bug,Q2_bug):
    ...
    dist = []
    for c in range(4):
        camera_index = combinations_4[0]
        d = camera_index[c]
        P_cam_0 = P[d][0].reshape(1, -1)  # Shape: (1, 4)
        P_cam_1 = P[d][1].reshape(1, -1)  # Shape: (1, 4)
        P_cam_2 = P[d][2].reshape(1, -1)  # Shape: (1, 4)
        
        P0_dot_Q = cp.einsum('ik,btk->bt', P_cam_0, Q4[:,:,0,:])  # Shape: (1, 22, 4)
        P1_dot_Q = cp.einsum('ik,btk->bt', P_cam_1, Q4[:,:,0,:])  # Shape: (250, 22, 4)
        P2_dot_Q = cp.einsum('ik,btk->bt', P_cam_2, Q4[:,:,0,:])  # Shape: (250, 22, 4)
        x.append(P0_dot_Q / P2_dot_Q)
        y.append(P1_dot_Q / P2_dot_Q)
        dist.append(cp.sqrt(cp.sum((Q4[:, :, 0, 0:3] - cam_coord[d]) ** 2, axis=-1)))
    ...
    for i in range(4):
        ...
        for c in range(3):
            ...
            P0_dot_Q = cp.einsum('ik,btk->bt', P_cam_0, Q3[:,:,i,:])  # Shape: (1, 22, 4)
    ...
    for i in range(6):
        ...
        for c in range(2):
            ...
            P0_dot_Q = cp.einsum('ik,btk->bt', P_cam_0, Q2[:,:,i,:])  # Shape: (1, 22, 4)
            P1_dot_Q = cp.einsum('ik,btk->bt', P_cam_1, Q2[:,:,i,:])
            P2_dot_Q = cp.einsum('ik,btk->bt', P_cam_2, Q2[:,:,i,:])
            x.append(P0_dot_Q / P2_dot_Q)
            y.append(P1_dot_Q / P2_dot_Q)
