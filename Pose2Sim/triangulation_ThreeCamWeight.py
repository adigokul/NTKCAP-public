...
def triangulation_from_best_cameras_weight(config, coords_2D_kpt, projection_matrices,body_name):
    ...
    sorted_idx = np.argsort(-likelihood_files)
    n_cams = len(x_files)
    error_min = np.inf
    ...
    if len(likelihood_files) < 4:
        id_cams_off = np.empty((1, 0), dtype=np.float64)
    else:
        id_cams_off = np.array([sorted_idx[-1]])
    ...
    all_error_noweight=[]
    residuals_SVD=[]
    nb_cams_excluded_final = 1
    id_excluded_cams_final = np.array([sorted_idx[-1]])
    return Q_final, id_excluded_cams_final, strongness_of_exclusion
...
def triangulate_all(config):
    ...
    for f in tqdm(range(*f_range)):
        ...
        # Triangulate cameras with min reprojection error
            ...
            residual_SVD_kpt = []            
            Q_kpt, id_excluded_cams_kpt, strongness_of_exclusion_kpt = triangulation_from_best_cameras_weight(config, coords_2D_kpt, P,keypoints_names[keypoint_idx])
