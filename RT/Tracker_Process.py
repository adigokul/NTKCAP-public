...
class Tracker_Process(Process):
    ...
    def run(self):
        ...
        shared_array_frame_show1 = np.ndarray((10,) + self.frame_shm_shape, dtype=np.uint8, buffer=existing_shm_frame_show1.buf)
        existing_shm_frame_show2 = shared_memory.SharedMemory(name=self.sync_frame_show_shm_name2)
        shared_array_frame_show2 = np.ndarray((10,) + self.frame_shm_shape, dtype=np.uint8, buffer=existing_shm_frame_show2.buf)
