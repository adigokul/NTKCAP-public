...
class SyncProcess(Process):
    def __init__(self, raw_frame_shm_name0, raw_frame_shm_name1, raw_frame_shm_name2, raw_frame_shm_name3, sync_frame_shm_name0, sync_frame_shm_name1, time_stamp_array_lst, camera_queue_lst, sync_frame_queue_lst, start_evt, stop_evt, *args, **kwargs):
        super().__init__()
    ...
    def run(self):
        ...
        shared_array_sync_frame0 = np.ndarray((self.buffer_length,) + self.sync_2frames_shm_shape, dtype=np.uint8, buffer=existing_shm_sync_frame0.buf)

        existing_shm_sync_frame1 = shared_memory.SharedMemory(name=self.sync_frame_shm_name1)
        shared_array_sync_frame1 = np.ndarray((self.buffer_length,) + self.sync_2frames_shm_shape, dtype=np.uint8, buffer=existing_shm_sync_frame1.buf)
        ...
        while self.start_evt.is_set():
            ...
            group1 = np.stack(raw_frame_all[:2], axis=0)
            group2 = np.stack(raw_frame_all[2:], axis=0)
            ...
            if self.check_follow_up(time_stamp_frame):
                ...
                np.copyto(self.shm_sync_frame_lst[0][idx, :], group1)
                np.copyto(self.shm_sync_frame_lst[1][idx, :], group2)
