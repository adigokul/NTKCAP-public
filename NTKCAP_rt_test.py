...
class GatherProcess(Process):
    def __init__(self, tracker_sync_shm_name0, tracker_sync_shm_name1, tracker_sync_shm_name2, tracker_sync_shm_name3, time_stamp_sync_array_lst, tracker_queue_lst, keypoints_sync_shm_name, start_evt, stop_evt, *args, **kwargs):
        ...
        self.buffer_length_tracker = 20
    def run(self):
        self.shm_tracker_lst.clear()
        existing_shm_tracker0 = shared_memory.SharedMemory(name=self.tracker_sync_shm_name0)
        shared_array_tracker0 = np.ndarray((self.buffer_length_tracker,) + self.tracker_sync_shm_shape, dtype=np.float32, buffer=existing_shm_tracker0.buf)
        existing_shm_tracker1 = shared_memory.SharedMemory(name=self.tracker_sync_shm_name1)
        shared_array_tracker1 = np.ndarray((self.buffer_length_tracker,) + self.tracker_sync_shm_shape, dtype=np.float32, buffer=existing_shm_tracker1.buf)
        existing_shm_tracker2 = shared_memory.SharedMemory(name=self.tracker_sync_shm_name2)
        shared_array_tracker2 = np.ndarray((self.buffer_length_tracker,) + self.tracker_sync_shm_shape, dtype=np.float32, buffer=existing_shm_tracker2.buf)
        existing_shm_tracker3 = shared_memory.SharedMemory(name=self.tracker_sync_shm_name3)
        shared_array_tracker3 = np.ndarray((self.buffer_length_tracker,) + self.tracker_sync_shm_shape, dtype=np.float32, buffer=existing_shm_tracker3.buf)
    ...
    async def read_from_queue(self, queue_id):
        
        while True:
            try:
                idx_get = self.tracker_q_lst[queue_id].get_nowait()
                return idx_get
            except Exception:
                return None
...
class Tracker_Process(Process):
    def __init__(self, raw_frame_shm, time_stamp_array, time_stamp_sync_array, tracker_sync_shm, cam_q, tracker_q, start_evt, stop_evt, *args, **kwargs):
        super().__init__()
    ...
    def run(self):
        ...
        shared_array_tracker = np.ndarray((self.buffer_length_tracker,) + self.tracker_sync_shm_shape, dtype=np.float32, buffer=existing_shm_tracker.buf)
        
        idx = 0
        while self.start_evt.is_set():
            try:
                idx_get = self.camera_q.get(timeout=0.05)
            except:
                time.sleep(0.01)
                continue
            ...
            idx = (idx+1) % self.buffer_length_tracker
            # scores = keypoints[..., 2]
...
class CameraProcess(Process):
    def __init__(self, cam_id, camera_shm, time_stamp_array, camera_queue, start_cam_evt, stop_cam_evt, *args, **kwargs):
        super().__init__()
...
class MainWindow(QMainWindow):
    ...
    def OpenCamera(self):
        ...
        for i in range(4):
            ...
            tracker_sync_shm = shared_memory.SharedMemory(create=True, size=int(np.prod(self.tracker_sync_shm_shape) * np.dtype(np.float32).itemsize * self.buffer_length_tracker))
