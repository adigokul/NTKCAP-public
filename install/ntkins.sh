...
# REASON: TensorRT may try to load CUDA 12 libraries based on driver version
#         reporting, even when CUDA 11.8 toolkit is installed.
# =============================================================================
create_cuda12_symlinks() {
    local cuda_lib="${CUDA_HOME}/lib64"

    # Check if CUDA 12 libraries already exist (CUDA 12 is installed)
    if [[ -f "${cuda_lib}/libcublasLt.so.12" ]]; then
        log "CUDA 12 libraries already exist - no symlinks needed"
        return 0
    fi

    # Check if we have CUDA 11 libraries to link from
    if [[ ! -f "${cuda_lib}/libcublasLt.so.11" ]]; then
        warn "CUDA 11 libraries not found at ${cuda_lib} - skipping symlink creation"
        return 1
    fi

    info "Creating CUDA 12 compatibility symlinks in ${cuda_lib}..."
    info "This is needed because NVIDIA driver may report CUDA 12/13 capability"
    info "but only CUDA 11.8 toolkit is installed."
    ...
    for lib_spec in "${libs[@]}"; do
        ...
        if [[ ! -f "${src}" ]]; then
            ...
            local found_src=$(find "${cuda_lib}" -maxdepth 1 -name "${basename}.so.*" ! -name "*.so.12" 2>/dev/null | head -1)
            if [[ -n "${found_src}" ]]; then
                src="${found_src}"
            else
                ((skipped++))
        ...
        if sudo ln -sf "$(basename "${src}")" "${dst}" 2>/dev/null; then
            log "Created: ${dst} -> $(basename "${src}")"
            ((created++))
        else
            warn "Failed to create symlink: ${dst} (may need sudo)"
        fi
    done

    if [[ ${created} -gt 0 ]]; then
        log "Created ${created} CUDA 12 compatibility symlinks"
...
# NOTE: TensorRT Python bindings will be installed AFTER TensorRT SDK is extracted (Step 3)
# This is because we need the wheel files from the SDK directory
...
# Uninstall headless version if present, then install full version with GUI
...
# ==============================================================================
# STEP 3.5: INSTALL TENSORRT PYTHON BINDINGS
# ==============================================================================
...
# =============================================================================
# STEP 1: ACTIVATE CONDA FIRST (needed for CONDA_PREFIX)
# =============================================================================
...
find_cuda() {
    ...
    for p in /usr/local/cuda /usr/local/cuda-11.8 /usr/local/cuda-*; do
...
# IMPORTANT: CONDA_PREFIX/lib MUST come first for libstdc++ compatibility
# =============================================================================

# Clean existing LD_LIBRARY_PATH of cuda/conda paths
CLEAN_LD_PATH=$(echo "${LD_LIBRARY_PATH:-}" | tr ':' '\n' | \
    grep -v "/cuda" | grep -v "/miniconda" | grep -v "/anaconda" | \
    grep -v "^$" | tr '\n' ':' | sed 's/:$//')

# Set paths - CONDA_PREFIX/lib first for newer libstdc++ (CXXABI compatibility)
export PATH="${CUDA_HOME}/bin:${PATH}"
