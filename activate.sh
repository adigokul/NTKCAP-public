...
# 
# This script sets up:
# - Qt platform configuration (for PyQt6 GUI)
# - CUDA paths
# - TensorRT paths  
# - MMDeploy SDK library paths
...
# ==============================================================================
# CUDA SETUP (auto-detect, no hardcoded paths)
# ==============================================================================
CUDA_SEARCH_PATHS=(
    "/usr/local/cuda-11.8"
    "/usr/local/cuda-12.0"
...
if [[ -n "$CUDA_HOME" ]]; then
    ...
    echo -e "${GREEN}✓ CUDA:${NC} $CUDA_VER at $CUDA_HOME"
else
    echo -e "${YELLOW}⚠ CUDA not found in standard paths${NC}"
fi

# ==============================================================================
# TENSORRT SETUP (relative to NTKCAP_ROOT)
# ==============================================================================
# Search for TensorRT in ThirdParty directory
THIRDPARTY_DIR="${NTKCAP_ROOT}/NTK_CAP/ThirdParty"
TENSORRT_DIR=""

# Try to find TensorRT directory (various versions)
for trt_dir in "${THIRDPARTY_DIR}"/TensorRT-*; do
    if [[ -d "$trt_dir" && -f "$trt_dir/lib/libnvinfer.so" ]]; then
        TENSORRT_DIR="$trt_dir"
        break
    fi
done

if [[ -n "$TENSORRT_DIR" && -d "$TENSORRT_DIR" ]]; then
    export TENSORRT_DIR="$TENSORRT_DIR"
    export TENSORRT_ROOT="$TENSORRT_DIR"
    export TRT_LIBPATH="${TENSORRT_DIR}/lib"
    export LD_LIBRARY_PATH="${TENSORRT_DIR}/lib:${LD_LIBRARY_PATH:-}"
    TRT_VER=$(basename "$TENSORRT_DIR" | sed 's/TensorRT-//')
    echo -e "${GREEN}✓ TensorRT:${NC} $TRT_VER"
else
    echo -e "${YELLOW}⚠ TensorRT not found in ${THIRDPARTY_DIR}${NC}"
...
# Method 1: Check for saved path from setup script
...
# Method 2: Check default build location (relative path)
if [[ "$MMDEPLOY_SDK_LOADED" == "false" ]]; then
    MMDEPLOY_BUILD_LIB="${MMDEPLOY_DIR}/build/lib"
...
if [[ "$MMDEPLOY_SDK_LOADED" == "false" ]]; then
    FOUND_LIB=$(find "${MMDEPLOY_DIR}" -name "libmmdeploy_trt_net.so" 2>/dev/null | head -1)
    if [[ -n "$FOUND_LIB" ]]; then
        MMDEPLOY_LIB_DIR=$(dirname "$FOUND_LIB")
        export LD_LIBRARY_PATH="${MMDEPLOY_LIB_DIR}:${LD_LIBRARY_PATH:-}"
        echo -e "${GREEN}✓ MMDeploy SDK:${NC} $MMDEPLOY_LIB_DIR"
        MMDEPLOY_SDK_LOADED=true
    fi
...
# ==============================================================================
# RTMLIB FALLBACK CHECK
# ==============================================================================
if [[ "${CONDA_DEFAULT_ENV}" == "ntkcap_env" ]]; then
    python -c "
try:
    from rtmlib import PoseTracker
    print('\033[0;32m✓ rtmlib:\033[0m Available as fallback')
except ImportError:
    print('\033[0;33m⚠ rtmlib:\033[0m Not installed (optional fallback)')
" 2>/dev/null
fi

# ==============================================================================
# SET WORKING DIRECTORY AND EXPORT PATHS
# ==============================================================================
cd "$NTKCAP_ROOT"
export NTKCAP_ROOT
export PYTHONPATH="${NTKCAP_ROOT}:${PYTHONPATH:-}"
